...
export class MeetupController {
  ...
  create(
    ...
    return this.meetupService.createMeetup(req.user.sub, body);
  }
  ...
  @Get('mine')
  mine(@Req() req: any) {
    return this.meetupService.getMyMeetups(req.user.sub);
  }

  @Get(':id')
  getOne(@Req() req: any, @Param('id') id: string) {
    return this.meetupService.getMeetup(id, req.user.sub);
  }

  @Post(':id/rsvp')
  rsvp(@Req() req: any, @Param('id') id: string, @Body() body: { status: 'going' | 'interested' | 'cancel' }) {
    return this.meetupService.rsvp(id, req.user.sub, body.status);
  }
}
